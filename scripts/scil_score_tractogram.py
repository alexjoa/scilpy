...
def _build_arg_parser():
    p = argparse.ArgumentParser(
        description="Scores input tractogram overall and bundlewise. Outputs \
            ...
            tractogram into resulting .trk : *_tc.trk, *_fc.trk, mc.trk and \
            *_wpc.trk, where * is the current bundle.\n\
            ...
            mc : missing connections, streamlines not joining two ROIs.\n \
...
def find_separate_roi(separate_roi, comb):
    if comb in separate_roi:
        return separate_roi.index(comb)
    return None
...
def main():
            ...
            # that correctly classify as tc and incorrectly as fc.
                ...
                for roi1_name in roi1_overlap_list:
                    for roi2_name in roi2_overlap_list:
                        is_tp1, tc_pos = find_tc_pos(tc_filenames,
                                                     (roi1_name[1],
                                                      roi2_name[1]))
                        if is_tp1:
                            fc_streamlines = \
    ...
    final_results = {}
    missing_conn_sft = StatefulTractogram.from_sft(missing_conn_streamlines,
                                                   sft)
